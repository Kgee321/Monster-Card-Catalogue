""" Component 5 -- Delete Cards (version 1)
This code is taking the code from in 09_search_cards_function_v1
and using it to find and delete items.
Most of the code already works for deleting a card,
but I made a few changes to the function so it better suits
deleting and adding some output statements to give clear
instruction to the user.
Dictionary printed at the end to make test if card did
delete.
Written by Katelyn
...
def search_delete(action, message, other_action):
    while True:
        ...
        # If card is correct
        if correct == "No":
            easygui.msgbox(f"Ok, Monster card was not deleted", "Card is Correct")
            break
